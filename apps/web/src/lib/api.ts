...
export const api = {
  ...
  async regenerateChat(chatId: string, input: RetryChatInput, onEvent: (event: NormalizedStreamEvent) => void, signal?: AbortSignal): Promise<void> {
    const response = await request(`/chats/${chatId}/regenerate`, {
      body: JSON.stringify(input),
      method: 'POST',
      signal,
    });
